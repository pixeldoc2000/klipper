...
xosc_setup(void)
{
    xosc_hw->startup = DIV_ROUND_UP(CONFIG_CLOCK_REF_FREQ, 1000 * 256); // 1ms
...
clock_setup(void)
{
    ...
    xosc_setup();
    unreset(RESETS_RESET_PLL_SYS_BITS);
    pll_setup(pll_sys_hw, 125, 125*CONFIG_CLOCK_REF_FREQ/FREQ_SYS);
    csys->ctrl = 0;
    ...
    // Enable GPIO control
    unreset(RESETS_RESET_IO_BANK0_BITS | RESETS_RESET_PADS_BANK0_BITS);
}
